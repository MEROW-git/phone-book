<x-app-layout>
    <div class="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
        <div class="mb-6 flex items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-slate-900 dark:text-slate-100">Add Contact</h1>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Create a contact with one or more phone numbers.</p>
            </div>
        ...
        <form method="POST" action="{{ route('contacts.store') }}" class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            @csrf
